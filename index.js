...
rama.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {                     
                    rama.sendMessage(anu.id, { text: `Hi👋 @${num.split("@")[0]}\nWelcome To ${metadata.subject}`, contextInfo:{"externalAdReply": {"title": `WELCOME`,"body": `👨‍💻`, "previewType": "PHOTO","thumbnailUrl": `ppuser`,"thumbnail": ``,"sourceUrl": "https://wa.me/message/4DCK3UGKERTGC1", mentionedJid: [num] }}})                  
                } else if (anu.action == 'remove') {
                    rama.sendMessage(anu.id, { text: `@${num.split("@")[0]} Leaving To ${metadata.subject}`, contextInfo:{"externalAdReply": {"title": `WELCOME`,"body": `👨‍💻`, "previewType": "PHOTO","thumbnailUrl": `ppuser`,"thumbnail": ``,"sourceUrl": "https://wa.me/message/4DCK3UGKERTGC1", mentionedJid: [num] }}})
                }
            }
        } catch (err) {
            console.log(err)
